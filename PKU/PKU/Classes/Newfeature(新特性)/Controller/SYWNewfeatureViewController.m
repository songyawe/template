...
#import "SYWNewfeatureViewController.h"
#import "SYWTabBarController.h"
#define SYWNewfeatureCount 4

@interface SYWNewfeatureViewController () <UIScrollViewDelegate>
@property (nonatomic, weak) UIPageControl *pageControl;

@property (nonatomic, weak) UIScrollView *scrollView;
@end

@implementation SYWNewfeatureViewController
- (void)viewDidLoad
{
    [super viewDidLoad];
    ...
    for (int i = 0; i<SYWNewfeatureCount; i++) {
...
#warning 就算不主动添加子控件到scrollView中，scrollView内部还是可能会有一些子控件
    ...
    scrollView.contentSize = CGSizeMake(SYWNewfeatureCount * scrollW, 0);
    ...
    pageControl.numberOfPages = SYWNewfeatureCount;
    pageControl.backgroundColor = [UIColor redColor];
    pageControl.currentPageIndicatorTintColor = SYWColor(253, 98, 42);
    pageControl.pageIndicatorTintColor = SYWColor(189, 189, 189);
...
- (void)startClick
{
    ...
    window.rootViewController = [[SYWTabBarController alloc] init];
    
    // modal方式，不建议采取：新特性控制器不会销毁
//    SYWTabBarViewController *main = [[SYWTabBarViewController alloc] init];
//    [self presentViewController:main animated:YES completion:nil];
}

- (void)dealloc
{
    SYWLog(@"SYWNewfeatureViewController-dealloc");
}
